...
public class LineService {
    ...
    public LineResponse findLineResponsesById(final Long id) {
        final Line line = lineRepository.findById(id).orElseThrow(RuntimeException::new);
        final List<Long> stationIds = line.getStationInOrder().stream()
                .map(it -> it.getStationId())
                .collect(Collectors.toList());

        final Map<Long, Station> stations = stationService.findStationsByIds(stationIds);

        final List<LineStationResponse> lineStationResponses = extractLineStationResponses(line, stations);
